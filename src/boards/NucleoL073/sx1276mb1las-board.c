...
//    }
    //According to Hardware layout, always return RF_PACONFIG_PASELECT_PABOOST
    return RF_PACONFIG_PASELECT_PABOOST;
}
...
int16_t SX1276ReadRssiFsk( uint32_t frequency )
{
    int16_t rssi;
    ...
    SX1276Write(REG_RXBW, 0x02); // 125k ref: FskBandwidths
    SX1276Write(REG_AFCBW, 0x02); // 125k ref: FskBandwidths
...
bool SX1276IsChannelFreeFsk(uint32_t freq, int16_t rssiThresh)
{
    if(rssiThresh > SX1276ReadRssiFsk(freq))
        return true;
    return false;
}
